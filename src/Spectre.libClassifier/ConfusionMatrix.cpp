...
ConfusionMatrix::ConfusionMatrix(const gsl::span<const Label> actual, const gsl::span<const Label> expected): ConfusionMatrix([&]()
{
    ...
    {
        throw core::exception::InconsistentArgumentSizesException("actual", actual.size(), "expected", expected.size());
    }
    ...
    for(auto i = 0; i < actual.size(); ++i)
    {
        if(actual[i] == POSITIVE)
        {
            ...
            {
                throw exception::NotABinaryLabelException(expected[i], i, "expected");
            }
        }
        ...
        {
            throw exception::NotABinaryLabelException(actual[i], i, "actual");
        }
    }
